"""
HA demo — master 2 of 3.

Run this + run_master1.py + run_master3.py in three terminals. They
...
ha = ClusterHA(
    node_id="master-2",
    peers=["http://127.0.0.1:7070", "http://127.0.0.1:7072"],
    election_timeout_sec=1.0,
    heartbeat_interval_sec=0.3,
    sync_interval_sec=2.0,
    state_file="ha_state_master2.json",   # survives a restart
)
...
print("master-2 running on :7071 — Ctrl-C to stop")
print("Check role anytime:  curl http://localhost:7071/v1/ha/status")
try:
    while True:
        time.sleep(1)
except KeyboardInterrupt:
    print("\nShutting down master-2...")
    master.stop()